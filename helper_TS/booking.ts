...
export class BookingHelper {
    ...
    async verifyGrandTotal(page: Page, singlePriceOfTicket: any) {

        const ticketCount = parseInt(await page.locator(".ticket-count").innerText());
        console.log(ticketCount);
        const totalTextgrid = page.locator(".p-4>div").filter({ hasText: "Total" })
        const totalText: any = await totalTextgrid.locator("span").nth(1).textContent();
    ...
    async createBookingFromFilters(page: Page, {
    ...
    async findBookingCardByRef(page: Page, bookingRef: any) {

        const matchingcard = await page.getByTestId("booking-card").filter({ hasText: bookingRef });
        await expect(matchingcard).toBeVisible();
        return matchingcard;
    }
    async openBookingDetailFromCard(page: Page, matchCard: Locator) {
